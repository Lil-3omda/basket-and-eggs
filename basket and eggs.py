...
egg1 = RECTA(120, 560, 150, 605)
egg2 = RECTA(50, 560, 80, 605)
...
def drawText(string, x, y):
    ...
    glTranslate(x, y, 0)
    glScale(0.25, 0.25, 1)
    string = string.encode()   # unicode   byte
    for c in string:
...
def Display():
    ...
    #       egg1
    glLoadIdentity()
    egg1.top = egg1.top + eggy1
    egg1.bottom = egg1.bottom + eggy1

    #       egg 2
    glLoadIdentity()
    egg2.top = egg2.top + eggy2
    egg2.bottom = egg2.bottom + eggy2

    DrawRectangle(egg1, 0)
    DrawRectangle(egg2, 0)

    if remain >= 0:
        #   egg 1

        if check_ball_bat(egg1, basket):
            x = randint(30, 570)
            egg1 = RECTA(x, 560, x + 30, 605)
            Score += 1
            if Score % 5 == 0:
                eggy1 -= 1
        elif egg1.bottom <= wall.bottom:
            x = randint(30, 600)
            egg1 = RECTA(x, 560, x + 30, 605)
            remain -= 1
        ...
        if check_ball_bat(egg2, basket):
            m = randint(30, 570)
            egg2 = RECTA(m, 560, m + 30, 605)
            Score += 1
            if Score % 7 == 0:
                eggy2 -= 1
        elif egg2.bottom <= wall.bottom:
            m = randint(30, 600)
            egg2 = RECTA(m, 560, m + 30, 605)
            remain -= 1
